...
unsigned long long const kKCLDefaultLogMaxFileSize      = 1024 * 1024;      // 1 MB
NSTimeInterval     const kKCLDefaultLogRollingFrequency = 60 * 60 * 24;     // 24 Hours
NSUInteger         const kKCLDefaultLogMaxNumLogFiles   = 5;                // 5 Files
unsigned long long const kKCLDefaultLogFilesDiskQuota   = 20 * 1024 * 1024; // 20 MB

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface KCLLogFileManagerDefault () {
...
@implementation KCLLogFileManagerDefault
...
- (instancetype)initWithLogsDirectory:(NSString *)aLogsDirectory {
    if ((self = [super init])) {
        _maximumNumberOfLogFiles = kKCLDefaultLogMaxNumLogFiles;
        _logFilesDiskQuota = kKCLDefaultLogFilesDiskQuota;
        ...
        NSLogVerbose(@"KCLFileLogManagerDefault: logsDirectory:\n%@", [self logsDirectory]);
        NSLogVerbose(@"KCLFileLogManagerDefault: sortedLogFileNames:\n%@", [self sortedLogFileNames]);
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(maximumNumberOfLogFiles))] ||
        ...
        NSLogInfo(@"KCLFileLogManagerDefault: Responding to configuration change: %@", keyPath);

        dispatch_async([KCLLog loggingQueue], ^{ @autoreleasepool {
 ...
 **/
- (void)deleteOldLogFiles {
    NSLogVerbose(@"KCLLogFileManagerDefault: deleteOldLogFiles");
    ...
    if (diskQuota) {
        ...
        for (NSUInteger i = 0; i < sortedLogFileInfos.count; i++) {
            KCLLogFileInfo *info = sortedLogFileInfos[i];
            used += info.fileSize;
    ...
    if (firstIndexToDelete == 0) {
        ...
        if (sortedLogFileInfos.count > 0) {
            KCLLogFileInfo *logFileInfo = sortedLogFileInfos[0];
    ...
    if (firstIndexToDelete != NSNotFound) {
        ...
        for (NSUInteger i = firstIndexToDelete; i < sortedLogFileInfos.count; i++) {
            KCLLogFileInfo *logFileInfo = sortedLogFileInfos[i];

            NSLogInfo(@"KCLLogFileManagerDefault: Deleting file: %@", logFileInfo.fileName);
...
- (NSString *)defaultLogsDirectory {
    ...
    NSString *kclLogsDirectoryName = @"KCLSDKLogs";
    NSString *logsDirectoryName = @"Logs";

#if TARGET_OS_IPHONE
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
    NSString *baseDir = ([paths count] > 0) ? [paths objectAtIndex:0] : nil;
    NSString *logsDirectory = [[baseDir stringByAppendingPathComponent:logsDirectoryName] stringByAppendingPathComponent:kclLogsDirectoryName];

#else
    NSString *appName = [[NSProcessInfo processInfo] processName];
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
    NSString *basePath = ([paths count] > 0) ? paths[0] : NSTemporaryDirectory();
    NSString *logsDirectory = [[[basePath stringByAppendingPathComponent:logsDirectoryName] stringByAppendingPathComponent:appName] stringByAppendingPathComponent:kclLogsDirectoryName];
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:_logsDirectory]) {
        ...
        if (![[NSFileManager defaultManager] createDirectoryAtPath:_logsDirectory
                                       withIntermediateDirectories:YES
                                                        attributes:nil
                                                             error:&err]) {
            NSLogError(@"KCLFileLogManagerDefault: Error creating logsDirectory: %@", err);
        }
    }
...
- (NSArray *)unsortedLogFileInfos {
    ...
    for (NSString *filePath in unsortedLogFilePaths) {
        KCLLogFileInfo *logFileInfo = [[KCLLogFileInfo alloc] initWithFilePath:filePath];
...
- (NSArray *)sortedLogFilePaths {
    ...
    for (KCLLogFileInfo *logFileInfo in sortedLogFileInfos) {
...
- (NSString *)createNewLogFile {
    ...
    do {
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
            NSLogVerbose(@"KCLLogFileManagerDefault: Creating new log file: %@", actualFileName);
...
@interface KCLLogFileFormatterDefault () {
    NSDateFormatter *_dateFormatter;
}

@end

@implementation KCLLogFileFormatterDefault
...
@interface KCLFileLogger () {
    __strong id <KCLLogFileManager> _logFileManager;
    
    KCLLogFileInfo *_currentLogFileInfo;
...
@implementation KCLFileLogger

- (instancetype)init {
    KCLLogFileManagerDefault *defaultLogFileManager = [[KCLLogFileManagerDefault alloc] init];

    return [self initWithLogFileManager:defaultLogFileManager];
}

- (instancetype)initWithLogFileManager:(id <KCLLogFileManager>)aLogFileManager {
    if ((self = [super init])) {
        _maximumFileSize = kKCLDefaultLogMaxFileSize;
        _rollingFrequency = kKCLDefaultLogRollingFrequency;
        _automaticallyAppendNewlineForCustomFormatters = YES;

        logFileManager = aLogFileManager;

        self.logFormatter = [KCLLogFileFormatterDefault new];
    }
...
- (void)scheduleTimerToRollLogFileDueToAge {
    ...
    NSLogVerbose(@"KCLFileLogger: scheduleTimerToRollLogFileDueToAge");

    NSLogVerbose(@"KCLFileLogger: logFileCreationDate: %@", logFileCreationDate);
    NSLogVerbose(@"KCLFileLogger: logFileRollingDate : %@", logFileRollingDate);
...
- (void)rollLogFileNow {
    NSLogVerbose(@"KCLFileLogger: rollLogFileNow");
...
- (void)maybeRollLogFileDueToAge {
    if (_rollingFrequency > 0.0 && _currentLogFileInfo.age >= _rollingFrequency) {
        NSLogVerbose(@"KCLFileLogger: Rolling log file due to age...");
...
- (void)maybeRollLogFileDueToSize {
    ...
    if (_maximumFileSize > 0) {
        ...
        if (fileSize >= _maximumFileSize) {
            NSLogVerbose(@"KCLFileLogger: Rolling log file due to size (%qu)...", fileSize);
...
- (KCLLogFileInfo *)currentLogFileInfo {
    if (_currentLogFileInfo == nil) {
        ...
        if ([sortedLogFileInfos count] > 0) {
            KCLLogFileInfo *mostRecentLogFileInfo = sortedLogFileInfos[0];
            ...
            if (!_doNotReuseLogFiles && !mostRecentLogFileInfo.isArchived && !shouldArchiveMostRecent) {
                NSLogVerbose(@"KCLFileLogger: Resuming logging with file %@", mostRecentLogFileInfo.fileName);
        ...
        if (_currentLogFileInfo == nil) {
            ...
            _currentLogFileInfo = [[KCLLogFileInfo alloc] initWithFilePath:currentLogFilePath];
        }
    }
    ...
    if (_currentLogFileHandle == nil) {
        ...
        if (_currentLogFileHandle) {
            ...
            dispatch_source_set_event_handler(_currentLogFileVnode, ^{ @autoreleasepool {
                                                                          NSLogInfo(@"KCLFileLogger: Current logfile was moved. Rolling it and creating a new one");
                                                                          [self rollLogFileNow];
                                                                      } });
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark KCLLogger Protocol
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (message) {
        ...
        @try {
            ...
            if (exception_count <= 10) {
                NSLogError(@"KCLFileLogger.logMessage: %@", exception);

                if (exception_count == 10) {
                    NSLogError(@"KCLFileLogger.logMessage: Too many exceptions -- will not log any more of them.");
                }
            }
        }
    }
...
#if TARGET_IPHONE_SIMULATOR
    NSString * const kKCLXAttrArchivedName = @"archived";
#else
    NSString * const kKCLXAttrArchivedName = @"lumberjack.log.archived";
#endif

@interface KCLLogFileInfo () {
...
- (BOOL)isArchived {
    ...
    return [self hasExtensionAttributeWithName:kKCLXAttrArchivedName];

#else

    return [self hasExtendedAttributeWithName:kKCLXAttrArchivedName];
...
- (void)setIsArchived:(BOOL)flag {
    ...
    if (flag) {
        [self addExtensionAttributeWithName:kKCLXAttrArchivedName];
    } else {
        [self removeExtensionAttributeWithName:kKCLXAttrArchivedName];
    }

#else

    if (flag) {
        [self addExtendedAttributeWithName:kKCLXAttrArchivedName];
    } else {
        [self removeExtendedAttributeWithName:kKCLXAttrArchivedName];
    }
...
- (void)renameFile:(NSString *)newFileName {
    ...
    if (![newFileName isEqualToString:[self fileName]]) {
        ...
        NSLogVerbose(@"KCLLogFileInfo: Renaming file: '%@' -> '%@'", self.fileName, newFileName);

        NSError *error = nil;

        if ([[NSFileManager defaultManager] fileExistsAtPath:newFilePath] &&
            ![[NSFileManager defaultManager] removeItemAtPath:newFilePath error:&error]) {
            NSLogError(@"KCLLogFileInfo: Error deleting archive (%@): %@", self.fileName, error);
        }

        if (![[NSFileManager defaultManager] moveItemAtPath:filePath toPath:newFilePath error:&error]) {
            NSLogError(@"KCLLogFileInfo: Error renaming file (%@): %@", self.fileName, error);
        }
...
- (void)addExtendedAttributeWithName:(NSString *)attrName {
    ...
    if (result < 0) {
        NSLogError(@"KCLLogFileInfo: setxattr(%@, %@): error = %s",
                   attrName,
                   self.fileName,
                   strerror(errno));
    }
...
- (void)removeExtendedAttributeWithName:(NSString *)attrName {
    ...
    if (result < 0 && errno != ENOATTR) {
        NSLogError(@"KCLLogFileInfo: removexattr(%@, %@): error = %s",
                   attrName,
                   self.fileName,
                   strerror(errno));
    }
...
- (BOOL)isEqual:(id)object {
    if ([object isKindOfClass:[self class]]) {
        KCLLogFileInfo *another = (KCLLogFileInfo *)object;
