...
static KCLLogLevel _captureLevel = KCLLogLevelVerbose;

#ifdef __IPHONE_8_0
    #define KCLASL_IOS_PIVOT_VERSION __IPHONE_8_0
#endif
#ifdef __MAC_10_10
    #define KCLASL_OSX_PIVOT_VERSION __MAC_10_10
#endif

@implementation KCLASLLogCapture
...
+ (void)initialize
{
    #if (defined(KCLASL_IOS_PIVOT_VERSION) && __IPHONE_OS_VERSION_MAX_ALLOWED >= KCLASL_IOS_PIVOT_VERSION) || (defined(KCLASL_OSX_PIVOT_VERSION) && __MAC_OS_X_VERSION_MAX_ALLOWED >= KCLASL_OSX_PIVOT_VERSION)
        #if __IPHONE_OS_VERSION_MIN_REQUIRED < KCLASL_IOS_PIVOT_VERSION || __MAC_OS_X_VERSION_MIN_REQUIRED < KCLASL_OSX_PIVOT_VERSION
...
+ (void)start {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
        [KCLASLLogCapture captureAslLogs];
    });
}
...
+ (void)configureAslQuery:(aslmsg)query {
    const char param[] = "7";  // ASL_LEVEL_DEBUG, which is everything. We'll rely on regular KCLlog log level to filter
...
+ (void)aslMessageRecieved:(aslmsg)msg {
    ...
    switch ([level intValue]) {
        ...
        case ASL_LEVEL_CRIT     : flag = KCLLogFlagError;    async = NO;  break;
        case ASL_LEVEL_ERR      : flag = KCLLogFlagWarning;  async = YES; break;
        case ASL_LEVEL_WARNING  : flag = KCLLogFlagInfo;     async = YES; break;
        case ASL_LEVEL_NOTICE   : flag = KCLLogFlagDebug;    async = YES; break;
        case ASL_LEVEL_INFO     :
        case ASL_LEVEL_DEBUG    :
        default                 : flag = KCLLogFlagVerbose;  async = YES;  break;
    }
    ...
    [KCLLog log:async message:logMessage];
}
    ...
    @autoreleasepool
    {
        ...
        notify_register_dispatch(kNotifyASLDBUpdate, &notifyToken, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^(int token)
        {
            ...
            @autoreleasepool
            {
                ...
                [KCLASLLogCapture configureAslQuery:query];
                ...
                {
                    [KCLASLLogCapture aslMessageRecieved:msg];
